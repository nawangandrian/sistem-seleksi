...
class Haladministrasi extends BaseController
{
    protected $db;
    ...
    {
        $data['content'] = 'administrasi/data';
        $data['judul'] = 'Administrasi';
        $data['administrasi'] = $this->model_crud->getAll('administrasi');

        echo view('_applayout', $data);
    }

    public function detail($id_administrasi)
    {  
        $this->_validasiku('edit');
        $data = [
            'content'      => 'administrasi/data',
            'judul'        => 'Administrasi',
            'administrasi'     => $this->model_crud->getAdministrasi(),
        ];
    ...
    private function _validasi($mode)
    {
        ...
        if ($mode == 'add') {
            ...
            // Untuk mode edit, Anda perlu mengambil data administrasi dari database
    ...
    public function toggle($getId)
    {
        ...
        return redirect()->to(site_url('haladministrasi'));
    }
    ...
    public function add()
    {
        ...
        if (!$this->validation->withRequest($this->request)->run()) {
            ...
            // Jika role adalah "administrasi", simpan data ke tabel administrasi
            if ($input['role'] === 'admin') {
                // Siapkan data untuk disimpan ke tabel administrasi
                $input_administrasi = [
                    'user_id'   => $userId,
                    'nama_administrasi'   => $input['nama'],
                    'alamat_administrasi'   => $input['alamat_administrasi'],
                    'no_telp_administrasi'   => $input['no_telp'],
                    'email_administrasi'   => $input['email'],
                    'nip_administrasi'    => $input['username']
                ];
        
                // Simpan data ke tabel administrasi
                $save_administrasi = $this->model_crud->insertkode('administrasi', $input_administrasi);
            }
        
            if ($save && (!$input['role'] === 'admin' || $save_administrasi)) {
                session()->setFlashdata('success', 'Selamat! Berhasil Menambah Data');
            } else {
                session()->setFlashdata('error', 'Gagal Menambah Data');
            }
            return redirect()->to(base_url('haladministrasi'));
        }        
    }
    ...
    public function edit($getId)
    {
        ...
        $id_administrasi = $input['id_administrasi'];

            $input_data = [
                'nama'          => $input['nama_administrasi'],
                'username'      => $input['username'],
                'email'      => $input['email'],
                'no_telp'       => $input['no_telp']
            ];
            ...
            $input_administrasi = [
                'user_id'   => $id_user,
                'nip_administrasi'    => $input['username'],
                'nama_administrasi'   => $input['nama_administrasi'],
                'alamat_administrasi'   => $input['alamat_administrasi'],
                'no_telp_administrasi'   => $input['no_telp'],
                'email_administrasi'   => $input['email']
            ];
            ...
            // Simpan data ke tabel administrasi
            $$update = $this->model_crud->updatekode('administrasi', 'id_administrasi', $id_administrasi, $input_administrasi);
    ...
    {
        $id = htmlentities($getId);
        $administrasi = $this->model_crud->detailkode('administrasi', 'id_administrasi', $id);
        $nip = $administrasi->nip_administrasi;
        $delete = $this->model_crud->deletekode('user', 'username', $nip);
        if ($this->model_crud->deletekode('administrasi', 'id_administrasi', $id)) {
