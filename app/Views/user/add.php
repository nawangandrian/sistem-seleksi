        ...
        <div class="card shadow-sm border-bottom-primary" style="margin-bottom: 1rem;">
            <div class="card-header py-3">
                <div class="row">
                    ...
                    <div class="col-auto">
                        <a href="<?= base_url('halpengguna') ?>" class="btn btn-sm btn-secondary btn-icon-split">
                            <span class="icon">
            ...
            <div class="card-body">
                <form action="<?= base_url('halpengguna/add') ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="custom-control custom-radio">
                            <input  value="kepala blk" type="radio" id="peserta" name="role" class="custom-control-input">
                            <label class="custom-control-label" for="peserta">Kepala BLK</label>
                        </div>
                    </div>
                </div>
                <br>
                <div class="row form-group" id="pelatihanInput" style="display:none;">
        ...
        $('#email').on('change', function () {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.exists) {
                        $('#error_message').text('Email sudah digunakan.');
        ...
        $('#username').on('change', function () {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.exists) {
                        $('#error_message1').text('Username sudah digunakan.');
                        $('button[type="submit"]').prop('disabled', true); // Nonaktifkan tombol submit
                    } else {
...
<script>
    $(document).ready(function () {
        // Tangani perubahan pada radio button "Role"
        $('input[name="role"]').change(function () {
            // Jika role yang dipilih adalah "Peserta", tampilkan input pelatihan, jika tidak, sembunyikan
            if ($(this).val() === 'peserta') {
                $('#pelatihanInput').show();
            } else {
                $('#pelatihanInput').hide();
